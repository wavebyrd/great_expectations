...
export default function GithubNavbarItem ({ owner, repository, className }) {
  ...
  return repository && (
    <a
      href={`https://github.com/${owner}/${repository}`} target='_blank'
      className={className + ' ' + styles.githubBadge + ' ' + (showGithubBadgeInfo ? styles.githubBadgeNoErrors : '')} rel='noreferrer'
    >
      <ThemedImage
        sources={{ dark: githubMarkDarkImg, light: githubMarkImg }} className={styles.githubMark}
        alt='Github Invertocat Logo'
      />
      {showGithubBadgeInfo && (<div className={styles.githubBadgeInfo}>
        <ThemedImage
          sources={{ dark: githubLogoDarkImg, light: githubLogoImg }} className={styles.githubLogo}
          alt='Github Logo'
        />
