...
GX provides several Expectations specifically designed for managing data volume, all of which can be added directly from the GX Cloud UI or using the GX Core Python library. Some of them can be automatically generated by enabling Anomaly Detection when you add a new Data Asset using GX Cloud.
...
When you [create a new Data Asset with GX Cloud](/cloud/data_assets/manage_data_assets.md#add-a-data-asset-from-an-existing-data-source), you can enable Anomaly Detection to automatically generate an instance of this Expectation that detects non-increasing volume over time.
:::
