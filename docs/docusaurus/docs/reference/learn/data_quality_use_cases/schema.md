...
GX offers a collection of Expectations for schema validation, all of which can be added directly from the GX Cloud UI or using the GX Core Python library. Some of them can be automatically generated by enabling Anomaly Detection when you add a new Data Asset using GX Cloud. 
