...
GX offers a collection of Expectations for schema validation, all of which can be added directly in GX Cloud or GX Core. Some of them can be automatically generated by enabling Anomaly Detection when you add a new Data Asset using GX Cloud. 
...
When you [create a new Data Asset with GX Cloud](/cloud/data_assets/manage_data_assets.md#add-a-data-asset-from-an-existing-data-source), you can enable Anomaly Detection to automatically generate this Expectation to test that columns don't diverge from the initial set over time.
:::
