...
```python title="Python" name="datasource_configuration_inferred_multi_batch_regex_pattern"
```
...
```python title="Python" name="group_names for inferred multi batch default_regex key"
```
...
```python title="Python" name="data connector config for inferred filesystem data connector multi batch"
```
